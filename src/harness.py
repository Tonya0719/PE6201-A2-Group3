...
def build_d2c_report_markdown(comparison: dict):
    ...
    lines.extend([
        ...
        "",
        (
            "The D2(c) controlled variable is the tool-call execution strategy. "
            "The sequential baseline allows at most one tool call per model turn "
            "with parallel execution disabled. The batched condition allows multiple "
            "independent tool calls in the same model turn with parallel execution enabled."
        ),
        "",
        (
