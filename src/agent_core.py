...
def run_agent(
    ...
    *,
    ...
    for turn in range(1, config.STEP_CAP + 1):
        ...
        # Hard budget ceiling: a model call may itself cross the ceiling.
        # Once that happens, do not execute any further tool or write action.
        if cost > config.BUDGET_USD:
            guards.append("BUDGET_CAP_REACHED_AFTER_MODEL_CALL")
            return {
                "case_id": claim_id, "status": "stopped",
                "failure_reason": "BUDGET_CAP_REACHED_AFTER_MODEL_CALL",
                "write_status": "not_attempted", "decision_source": None,
                "turns": tool_turns, "tool_turns": tool_turns, "model_calls": turn,
                "input_tokens": inp, "output_tokens": out, "cost": cost,
                "tool_history": history, "guardrail_events": guards,
        ...
        if gated_calls:
            ...
            call = gated_calls[0]
            case_ok, case_reason = check_case_id_match(call, claim_id)
            if not case_ok:
                guards.append({"event": case_reason, "tool_call": call})
                return {
                    "case_id": claim_id, "status": "failed",
                    "failure_reason": case_reason,
                    "write_status": "not_attempted", "decision_source": None,
                    "turns": tool_turns, "tool_turns": tool_turns, "model_calls": turn,
                    "input_tokens": inp, "output_tokens": out, "cost": cost,
                    "tool_history": history, "guardrail_events": guards,
            ...
            if not ready:
                guards.append({"event": ready_reason, "tool_call": call})
                return {
                    "case_id": claim_id, "status": "failed",
        ...
        active_claim = _extract_latest_claim(history)
        intake_allowed, intake_blocked, intake_reason = check_no_retrieval_after_decisive_intake(raw_calls, active_claim)
        if intake_blocked:
            guards.append({
                "event": "READ_AFTER_DECISIVE_INTAKE_BLOCKED",
                "decisive_reason": intake_reason,
                "calls": intake_blocked,
            })

        calls, blocked = check_duplicate_actions(intake_allowed, seen)
        if blocked:
            guards.append({"event": "DUPLICATE_ACTION_BLOCKED", "calls": blocked})
        ...
        if approved:
            tool_turns += 1
        observations = execute_tool_calls(approved, parallel=parallel, tool_spec_version=spec_version)
        if intake_blocked:
            observations.extend([
                {
                    "tool": call.get("name"),
                    "ok": False,
                    "error": "READ_AFTER_DECISIVE_INTAKE_BLOCKED",
                    "decisive_reason": intake_reason,
                }
                for call in intake_blocked
            ])
