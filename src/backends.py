...
def get_openrouter_credit_balance():
    ...
    try:
        ...
        raise RuntimeError(
            f"OpenRouter credits request failed: HTTP {e.code} {detail}"
        ) from e
    except URLError as e:
        raise RuntimeError(
            f"OpenRouter credits request failed: {e.reason}"
        ) from e
...
def _pa_records(result: dict):
    """Normalize the D2(b) v1/v2 PA return-shape difference."""
    if "valid_records" in result:
        return result.get("valid_records", [])

    if "records" in result:
        return result.get("records", [])

    return result.get("matching_records", [])


def _uses_v2_preauth(messages: list[dict]):
    """Infer whether the current prompt exposes the V2 PA interface.

    Experiment runners may override tool_spec_version without changing the
    global config, so the scripted backend reads the actual system prompt.
    """
    for m in messages:
        ...
        if (
            "get_preauthorisation" in text
            and "date_of_service" in text
            and "valid_records" in text
        ):
            return True
...
def _scripted(messages: list[dict]):
    ...
    if (
        decision_args
        and any(
            o.get("tool") == "issue_decision_letter"
            for o in obs
        )
    ):
        return {
    ...
    if not by_tool.get("get_claim"):
        user = next(
            (
                ...
                if (
                    m.get("role") == "user"
                    and m.get("content", "").startswith("Process claim")
                )
            ),
            "",
        )
    ...
    if claim.get("duplicate_of"):
        payload = _issue_decision_action(
            ...
            [
                {
                    "source": "get_claim",
                    "fact": f"duplicate_of={claim['duplicate_of']}",
                }
            ],
        )
    ...
    # -----------------------------------------------------------------
    # Gate 2: hostile/injected narrative
    # -----------------------------------------------------------------

    narrative = (claim.get("narrative") or "").lower()

    if (
        "system note:" in narrative
        or "system audit override" in narrative
        or ("returned:" in narrative and "[" in narrative)
        or "ignore prior instructions" in narrative
        or "ignore previous instructions" in narrative
    ):
        payload = _issue_decision_action(
            claim["claim_id"],
            "escalate",
            ...
            [
                {
                    "source": "get_claim",
                    "fact": "hostile instruction in member narrative",
                }
            ],
        )
    ...
    # Prefer the deterministic arithmetic returned by get_claim.
    # The fallback preserves compatibility with older claim observations.
    total = float(
        claim.get(
            "claim_total",
            sum(float(x["amount"]) for x in claim["lines"]),
        )
    )

    # -----------------------------------------------------------------
    # Gate 3: lapsed policy
    # -----------------------------------------------------------------
            ...
            [
                {
                    ...
                    "fact": f"{pol['policy_id']} status=lapsed",
                }
            ],
        )
    ...
    if not (
        ...
        payload = _issue_decision_action(
            ...
            None,
            "Service date is outside policy dates.",
            [
                {
                    "source": "lookup_policy",
                    "fact": (
                        f"{dos} outside "
                        f"{pol['start_date']}..{pol['end_date']}"
                    ),
                }
            ],
        )
    ...
    if total > float(pol["remaining_annual_limit"]):
        payload = _issue_decision_action(
            ...
            None,
            "Claim total exceeds remaining annual limit.",
            [
                {
                    "source": "lookup_policy",
                    "fact": (
                        f"claim_total={total:g}, "
                        f"remaining={pol['remaining_annual_limit']}"
                    ),
                }
            ],
        )
    ...
    coverage_by_code = {
        x["procedure_code"]: x
        for x in by_tool.get("check_coverage", [])
    }

    pa_by_code = {
        x["procedure_code"]: x
        for x in by_tool.get("get_preauthorisation", [])
    }

    missing_cov = [
        x["code"]
        for x in claim["lines"]
        if x["code"] not in coverage_by_code
    ]
    ...
    missing_pa = [
        ...
        if c not in pa_by_code
    ]

    if missing_pa:
        use_v2 = _uses_v2_preauth(messages)
        ...
        for c in missing_pa:
            ...
            # Only V2 gets the stronger date-aware interface.
            if use_v2:
                arguments["date_of_service"] = dos
    ...
    # -----------------------------------------------------------------
    # Wait until runtime helper facts have been supplied
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # Gate 6a: missing required document
    # -----------------------------------------------------------------

    missing_docs = helper_context.get(
        "missing_documents",
        [],
    )
    ...
    if missing_docs:
        ...
        item = (
            f"{m['document'].replace('_', ' ')} "
            f"for line {m['procedure_code']}"
        )
        ...
        payload = _issue_decision_action(
            ...
            # ASK uses missing_item, not an escalation trigger.
            None,

            item,
            f"Required document is missing: {item}.",
            [
                {
                    "source": "check_missing_documents",
                    "fact": item,
                }
            ],
        )
    ...
    # -----------------------------------------------------------------
    # Gate 6b: missing / invalid required PA
    # -----------------------------------------------------------------

    for c in required_pa:
        pa_result = pa_by_code[c]

        # -----------------------------
        # V2
        # -----------------------------
        if "valid_records" in pa_result:
            valid_records = pa_result.get("valid_records", [])
            records_found = pa_result.get("records_found", 0)
            ...
            if valid is None:
                if records_found == 0:
                    item = (
                        f"pre-authorisation reference for line "
                        f"{c}, valid on {dos}"
                    )
                    fact = (
                        f"no matching preauthorisation exists "
                        f"for {c}"
                    )
                else:
                    item = (
                    ...
                    fact = (
                        f"{records_found} matching preauthorisation "
                        f"record(s) found, but none valid on {dos}"
                    )
        ...
        # V1
        # -----------------------------
        raw = _pa_records(pa_result)

        valid = next(
            (
                x
                for x in raw
                if x["valid_from"] <= dos <= x["valid_to"]
            ),
            None,
        )
        ...
        if valid is None:
            if not raw:
                ...
                fact = (
                    f"no matching preauthorisation for {c}"
                )
            else:
                item = (
                    f"current pre-authorisation for line "
                ...
                row = raw[0]
                fact = (
                    f"{row['preauthorisation_id']} valid "
    ...
    for line in claim["lines"]:
        ...
        if cov["coverage_status"] == "excluded":
            ...
            evidence.append(
                {
                    ...
                    "fact": (
                        f"{line['code']} excluded: "
                        f"{cov.get('exclusion_code')}"
                    ),
                }
            )
            ...
            if line["code"] in required_pa:
                pa_result = pa_by_code[line["code"]]

                if "valid_records" in pa_result:
                    valid = (
                        pa_result["valid_records"][0]
                        if pa_result["valid_records"]
                        else None
                    )
                else:
                    raw = _pa_records(pa_result)
                ...
                if valid:
                    fact += (
                        f"; preauth "
                        f"{valid['preauthorisation_id']} valid"
                    )
    ...
    if helper_context.get("hospital_panel") is False:
        evidence.append(
            {
                "source": "check_hospital_panel",
                "fact": f"{claim['hospital_id']} is non-panel",
            }
        )
    ...
    payload = _issue_decision_action(
        ...
        None,
        (
            f"All lines resolved. "
            f"approved_total={approved_total:g}; "
            f"refused_total={refused_total:g}."
        ),
        evidence,
    )
